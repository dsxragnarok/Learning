...
/**
  * Define an object to hold all our images for the game so images are only
  * ever created once. This type of object is known as a singleton.
  */
...
/**
	...
	* object. The background is drawn on the "background" canvas and creates
	* the illusion of moving by panning the image.
	*/
...
/**
  * Custom Pool object. Holds objects to be managed to prevent garbage collection
  */
...
function Pool(maxSize) {
	...
	/*
	 * Populates the array with Bullet objects
	 */
	this.init = function(object) {
	...
	/*
	 * Grabs the last item in the list and initializes it and pushes it to the front 
	 * of the array.
	 */
	...
	/*
	 * Used for the ship to be able to get two bullets at once. If only the get()
	 * function i used twice, the ship is able to fire and only have 1 bullet spawn
	 * instead of 2.
	 */
	...
	/*
	 * Draw any in use Bullets. If a bullet goes off the screen, clears it and pushes
...
function Bullet(object) {
	 ...
	 * Returns true if the bullet moved off the screen, indicating that the
	 * bullet is ready to be cleared by the pool, otherwise draws the
	 * bullet.
	 */
	 this.draw = function() {
...
function Ship() {
	...
	this.move = function() {
		...
		{
			// The ship moved, so erase it's current image so it can be
			// redrawn in its new location
			this.context.clearRect(this.x, this.y, this.width, this.height);
			
			// Update x and y according to the direction to move and redraw
			// the ship. Change the else-if to if statements
...
// Set Ship to inherit properties from Drawable
...
function Game() {
	/*
	 * Gets canvas information and context and sets up all the game objects.
	 * Returns true if the canvas is supported and false if it is not.
	 * This is to stop the animation script from constantly running on
	 * browsers that do not support the canvas.
	 */
	this.init = function() {
		...
		// Test to see if canvas is supported. Only need to check on canvas
		if( this.bgCanvas.getContext )
		{
			...
			this.ship.init(this.shipStartX, 
							this.shipStartY, 
...
/**
  * Sets up the document to listen to onkeydown events (fired when any key on
  * the keyboard is pressed down). When a key is pressed, it sets the
  * appropriate direction to true to let us know which key it was.
  */
document.onkeydown = function(e) {
	// Firefox and opera use charCode instead of keyCode to return
...
/**
  * Sets up the document to listen to onkeyup events (fired when any key on
  * the keyboard is released). When a key is released, it sets the appropriate
  * direction to false to let us know which key it was.
  */
...
function QuadTree(boundBox, lvl) {
	...
	/*
	 * Determine which node the objects belong to. -1 means object cannot 
